...
describe('Starting tests for default-angular-app', function() {
  ...
  it('website title should be default-angular-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('default-angular-app');
    })
  });

  it('network-name should be brian-network@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('brian-network@0.0.1.bna');
    });
  });

  it('navbar-brand should be default-angular-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('default-angular-app');
    });
  });
